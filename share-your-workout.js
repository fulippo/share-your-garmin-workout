...
class GarminImport {
	...
	static createWorkoutButtonSelector = '.create-workout';
	...
	static getCreateWorkoutButton() {
		...
		return Array.from(candidates).find(b => {
			...
			return text && targetPattern.test(text);
		});
	}
	...
	static injectImportButton(workoutText) {
		...
		const createButton = GarminImport.getCreateWorkoutButton();
		if (!createButton) {
		...
		if (createButton.nextSibling?.id === 'garmin-share-workout-file') {
			return;
		}

		let importButton = createButton.cloneNode(true);
		...
		createButton.parentNode.insertBefore(importButton, createButton.nextSibling);
		createButton.parentNode.insertBefore(inputFile, createButton.nextSibling);
	}
